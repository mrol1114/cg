#pragma once

#include <vector>
#include <cmath>
#include <windows.h>

#include <stdexcept>

#include <windef.h>
#include <GL/glew.h>
#include <GL/gl.h>

#include <GL/wglext.h>

#include <GL/glu.h>
#include <GLFW/glfw3.h>

#include <glm/ext/matrix_clip_space.hpp>
#include <glm/ext/matrix_transform.hpp>
#include <glm/ext.hpp>

#include "IDrawable.h"

// смежные грани в разных цветах
class SnubDodecahedron : public IDrawable
{
public:
	void Draw()override
	{
		DrawPolygons();
		DrawEdges();
	}

private:
	void DrawPolygons()
	{
		for (const auto& face : m_faces)
		{
			constexpr glm::vec4 defaultColor{0.4, 1, 0.4, 0};
			glColor4fv(glm::value_ptr(defaultColor));

			std::vector<glm::vec3> verticles;

			for (const auto& verticleIndex : face)
			{
				verticles.push_back(glm::make_vec3(m_verticles[verticleIndex].data()));
				verticles.back() *= 0.5;
			}

			glBegin(GL_POLYGON);

			for (const auto& verticle : verticles)
			{
				glVertex3fv(glm::value_ptr(verticle));
			}

			glEnd();
		}
	}

	void DrawEdges()
	{
		for (const auto& egde : m_edges)
		{
			constexpr glm::vec4 defaultColor{ 0, 0, 0, 0 };
			glColor4fv(glm::value_ptr(defaultColor));
			std::vector<glm::vec3> verticles;

			for (const auto& verticleIndex : egde)
			{
				verticles.push_back(glm::make_vec3(m_verticles[verticleIndex].data()));
				verticles.back() *= 1 * 0.5;
			}

			glLineWidth(3);

			glBegin(GL_LINES);

			for (const auto& verticle : verticles)
			{
				glVertex3fv(glm::value_ptr(verticle));
			}

			glEnd();
		}
	}

	std::vector<std::vector<double>> m_verticles = { {0,0,1.028031},{0.4638569,0,0.9174342},{0.2187436,0.4090409,0.9174342},{-0.2575486,0.3857874,0.9174342},{-0.4616509,-0.04518499,0.9174342},{-0.177858,-0.4284037,0.9174342},{0.5726782,-0.4284037,0.7384841},{0.8259401,-0.04518499,0.6104342},{0.6437955,0.3857874,0.702527},{0.349648,0.7496433,0.6104342},{-0.421009,0.7120184,0.6104342},{-0.6783139,0.3212396,0.702527},{-0.6031536,-0.4466658,0.702527},{-0.2749612,-0.7801379,0.6104342},{0.1760766,-0.6931717,0.7384841},{0.5208138,-0.7801379,0.4206978},{0.8552518,-0.4466658,0.3547998},{1.01294,-0.03548596,0.1718776},{0.7182239,0.661842,0.3208868},{0.3633691,0.9454568,0.1758496},{-0.04574087,0.9368937,0.4206978},{-0.4537394,0.905564,0.1758496},{-0.7792791,0.5887312,0.3208868},{-0.9537217,0.1462217,0.3547998},{-0.9072701,-0.3283699,0.3547998},{-0.6503371,-0.7286577,0.3208868},{0.08459482,-0.9611501,0.3547998},{0.3949153,-0.9491262,-0.007072558},{0.9360473,-0.409557,-0.1136978},{0.9829382,0.02692292,-0.2999274},{0.9463677,0.4014808,-0.007072558},{0.6704578,0.7662826,-0.1419366},{-0.05007646,1.025698,-0.04779978},{-0.4294337,0.8845784,-0.2999274},{-0.9561681,0.3719321,-0.06525234},{-1.022036,-0.1000338,-0.04779978},{-0.8659056,-0.5502712,-0.06525234},{-0.5227761,-0.8778535,-0.1136978},{-0.06856319,-1.021542,-0.09273844},{0.2232046,-0.8974878,-0.4489366},{0.6515438,-0.7200947,-0.3373472},{0.7969535,-0.3253959,-0.5619888},{0.8066872,0.4395354,-0.461425},{0.4468035,0.735788,-0.5619888},{0.001488801,0.8961155,-0.503809},{-0.3535403,0.6537658,-0.7102452},{-0.7399517,0.5547758,-0.4489366},{-0.9120238,0.1102196,-0.461425},{-0.6593998,-0.6182798,-0.4896639},{-0.2490651,-0.8608088,-0.503809},{0.4301047,-0.5764987,-0.734512},{0.5057577,-0.1305283,-0.8854492},{0.5117735,0.3422252,-0.8232973},{0.09739587,0.5771941,-0.8451093},{-0.6018946,0.2552591,-0.7933564},{-0.6879024,-0.2100741,-0.734512},{-0.3340437,-0.5171509,-0.8232973},{0.08570633,-0.3414376,-0.9658797},{0.1277354,0.1313635,-1.011571},{-0.3044499,-0.06760332,-0.979586} };
	std::vector<std::vector<double>> m_faces = { {0,1,2},{0,2,3},{0,3,4},{0,4,5},{1,6,7},{1,7,8},{1,8,2},{2,8,9},{3,10,11},{3,11,4},{4,12,5},{5,12,13},{5,13,14},{6,14,15},{6,15,16},{6,16,7},{7,16,17},{8,18,9},{9,18,19},{9,19,20},{10,20,21},{10,21,22},{10,22,11},{11,22,23},{12,24,25},{12,25,13},{13,26,14},{14,26,15},{15,26,27},{16,28,17},{17,28,29},{17,29,30},{18,30,31},{18,31,19},{19,32,20},{20,32,21},{21,32,33},{22,34,23},{23,34,35},{23,35,24},{24,35,36},{24,36,25},{25,36,37},{26,38,27},{27,38,39},{27,39,40},{28,40,41},{28,41,29},{29,42,30},{30,42,31},{31,42,43},{32,44,33},{33,44,45},{33,45,46},{34,46,47},{34,47,35},{36,48,37},{37,48,49},{37,49,38},{38,49,39},{39,50,40},{40,50,41},{41,50,51},{42,52,43},{43,52,53},{43,53,44},{44,53,45},{45,54,46},{46,54,47},{47,54,55},{48,55,56},{48,56,49},{50,57,51},{51,57,58},{51,58,52},{52,58,53},{54,59,55},{55,59,56},{56,59,57},{57,59,58},{0,5,14,6,1},{2,9,20,10,3},{4,11,23,24,12},{7,17,30,18,8},{13,25,37,38,26},{15,27,40,28,16},{19,31,43,44,32},{21,33,46,34,22},{29,41,51,52,42},{35,47,55,48,36},{39,49,56,57,50},{45,53,58,59,54} };
	std::vector<std::vector<double>> m_edges = { {42,30},{18,30},{34,22},{47,55},{6,7},{40,27},{8,9},{16,15},{33,44},{57,58},{33,21},{50,39},{43,31},{12,5},{10,3},{26,27},{58,53},{5,14},{9,20},{26,13},{42,31},{48,36},{25,13},{36,37},{25,37},{22,23},{49,38},{10,20},{59,55},{12,13},{26,38},{10,11},{42,43},{44,45},{0,3},{59,54},{24,35},{46,54},{9,18},{24,25},{17,29},{34,35},{53,45},{44,53},{19,31},{49,37},{16,17},{57,59},{37,38},{25,12},{8,2},{41,29},{56,57},{24,12},{17,7},{24,23},{56,49},{33,45},{11,22},{16,6},{58,51},{58,59},{0,2},{16,7},{40,28},{35,36},{57,50},{54,55},{56,59},{43,44},{38,39},{40,41},{18,19},{1,7},{13,14},{30,31},{8,18},{16,28},{32,21},{35,23},{28,29},{54,47},{52,53},{0,5},{13,5},{10,21},{27,38},{50,51},{25,36},{48,37},{48,49},{11,23},{41,51},{3,4},{57,51},{51,52},{2,3},{1,6},{18,31},{14,15},{20,21},{56,55},{35,47},{0,4},{34,46},{10,22},{12,4},{26,14},{9,2},{49,39},{40,39},{8,1},{19,20},{45,54},{0,1},{32,33},{27,15},{1,2},{41,28},{45,46},{58,52},{29,30},{40,50},{17,30},{9,19},{41,50},{11,4},{32,19},{33,46},{42,52},{17,28},{34,47},{43,52},{46,47},{32,44},{8,7},{56,48},{34,23},{24,36},{42,29},{3,11},{48,55},{43,53},{32,20},{26,15},{6,15},{21,22},{27,39},{14,6},{4,5} };
};